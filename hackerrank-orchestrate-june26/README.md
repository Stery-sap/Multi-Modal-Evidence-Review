...
```bash
pip install google-genai openai anthropic python-dotenv pandas pillow
```

### 2. Configure API Keys
Set your LLM provider keys in your environment variables or in a `.env` file at the project root:
```bash
# Set either of the following keys:
export GEMINI_API_KEY="your-gemini-api-key"
export OPENAI_API_KEY="your-openai-api-key"
```
*Note: If no API keys are present (or if `MOCK_VLM=true` is set), the system will run in heuristic fallback mock mode to allow testing the pipeline's logic and verifying the CSV structure without invoking remote endpoints.*

### 3. Run Evaluation Loop
To run the evaluation comparisons against the labeled ground truth in `dataset/sample_claims.csv`:
```bash
python code/evaluation/main.py
```

### 4. Run Claims Verification Pipeline
To execute the pipeline on the test set (`dataset/claims.csv`) and output predictions to `output.csv`:
```bash
python code/main.py
```
...
The evaluation suite compares Strategy A (`gemini-2.5-flash`) and Strategy B (`gpt-4o-mini`) on `dataset/sample_claims.csv`. Metrics and details of the comparison can be found in the report:
[evaluation_report.md](file:///c:/Users/stery/OneDrive/Documents/Multi-Modal%20Evidence%20Review/hackerrank-orchestrate-june26/code/evaluation/evaluation_report.md).
